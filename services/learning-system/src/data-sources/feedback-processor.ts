    ...
    try {
      ...
      for (const knowledge of relatedKnowledge) {
        const newConfidence = Math.max(
          0,
          Math.min(1, knowledge.confidence + confidenceAdjustment)
        );
        ...
        console.log(
          `[LEARNING] Adjusted confidence of knowledge ${knowledge.id} from ${knowledge.confidence} to ${newConfidence} based on feedback rating ${feedback.rating}`
        );
      }
    } catch (error) {
    ...
    try {
      ...
      const feedbackKnowledge = await models.Knowledge.create({
        ...
        tags: [
          ...
          feedback.Message
            ? `conversation:${feedback.Message.conversationId}`
            : "",
        ],
      });
