...
//    */
//   async generateResponse(userMessage: string, context: any): Promise<{
//     responseText: string;
//     usedKnowledge: any[];
...
//       const relevantKnowledge = await this.findRelevantKnowledge(keywords, context, userMessage);
...
//         const templateResult = await this.tryUseTemplate(userMessage, relevantKnowledge, context);
...
//           const generatedResponse = this.generateDirectResponse(userMessage, relevantKnowledge, context);
...
//           content: this.parseKnowledgeContent(k.content),
//           type: k.type,
...
//         try {
//           const renderedResponse = templateEngine.render(template.template, templateData);
...
//           }
//         } catch (templateError) {
//           console.error(`[RESPONSE] Error rendering template ${template.id}:`, templateError);
...
//       const sortedKnowledge = [...knowledge].sort((a, b) => b.confidence - a.confidence);
      
//       // Get high-confidence knowledge
//       const highConfidenceKnowledge = sortedKnowledge.filter(k => k.confidence > 0.7);
...
//       const mediumConfidenceKnowledge = sortedKnowledge.filter(k => k.confidence > 0.4);
...
//   ): string {
//     // Simple implementation - in a real system, this would use NLG
...
//     const knowledgeTypes = new Set(topKnowledge.map(k => k.type));
    
//     if (knowledgeTypes.size === 1) {
//       const type = topKnowledge[0].type;
      
//       switch (type) {
//         case 'fact':
//           // For facts, just join them
//           response = topKnowledge.map(k => this.cleanContent(k.content)).join('. ');
//           break;
          
//         case 'entity':
//           // For entities, format as descriptions
//           response = topKnowledge.map(k => {
//             const entity = this.parseKnowledgeContent(k.content);
//             return `${entity.text} is ${entity.type === 'unknown' ? 'a' : `a ${entity.type}`}`;
//           }).join('. ');
//           break;
...
//           response = topKnowledge.map(k => {
//             const concept = this.parseKnowledgeContent(k.content);
//             return `${concept.text} is an important concept`;
//           }).join('. ');
//           break;
...
//         const contextResponses = defaultResponses.filter(r => r.context === context);
...
//           return contextResponses[randomIndex].responseText;
//         }
        
//         // If no context-specific responses, use general ones
//         const generalResponses = defaultResponses.filter(r => !r.context || r.context === 'general');
...
//           return generalResponses[randomIndex].responseText;
//         }
//       }
...
//         return dbResponses[0].responseText;
//       }
