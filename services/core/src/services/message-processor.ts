...
const learningServiceUrl =
  process.env.LEARNING_SYSTEM_URL || "http://learning-system:3002";
const responseServiceUrl =
  process.env.RESPONSE_GENERATOR_URL || "http://response-generator:3003";
const egoServiceUrl = process.env.EGO_SERVICE_URL || "http://ego-service:3004";
...
class MessageProcessor {
  ...
  async processMessage(
    ...
    try {
      ...
      await this.processWithLearningSystem(
        message,
        task,
        userId,
        conversationId
      ...
      this.triggerEgoProcessing(message, userId, conversationId).catch((err) =>
        console.error("Error triggering ego processing:", err)
      );
    } catch (error) {
   ...
   */
  private async processWithLearningSystem(
    message: any,
    task: any,
    userId: number,
    conversationId: number
  ...
  private async generateResponse(
    ...
    try {
      ...
      const response = await axios.post(`${responseServiceUrl}/api/generate`, {
        messageId: message.id || message.dataValues?.id,
        conversationId,
        userId,
        content: message.content || message.dataValues?.content,
   ...
   */
  private async triggerEgoProcessing(
    message: any,
    userId: number,
    conversationId: number
  ): Promise<void> {
